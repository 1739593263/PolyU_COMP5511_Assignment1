...
"""
calculation without CV matrix
"""
class timeCons1(ea.Problem):
    def __init__(self):
    ...
    def aimFunc(self, pop):
        ...
        for i in range(pop.sizes):
            ...
            ObjV_cv.append(round(constraint, 3))
            time = time+distance
            
        pop.ObjV = np.hstack([np.array([ObjV_dist]).T, np.array([ObjV_pro]).T, np.array([ObjV_cv]).T])
